...
class CustomLine:
    ...
    @staticmethod
    def is_box_in_line(
        line: "CustomLine", bbox: BBox, height_line_tolerance: float
    ) -> bool:
        """
...
class CustomLineV1(CustomLine):
    ...
    def _find_best_anchor(
        anchors: Sequence[str], fields: Dict[str, ListFieldV1]
    ) -> str:
        """
        ...
        for field in anchors:
            values: List[ListFieldValueV1] = fields[field].values
    ...
    def prepare(
        ...
        if anchor_field and len(anchor_field.values) > 0:
            ...
            for i in range(1, len(anchor_field.values)):
                ...
                if not CustomLine.is_box_in_line(
    ...
    def get_line_items(
        ...
        anchor = CustomLineV1._find_best_anchor(anchors, fields_to_transform)
        if not anchor:
            print(Warning("Could not find an anchor!"))
            return line_items
        lines_prepared: List[CustomLineV1] = CustomLineV1.prepare(
...
class CustomLineV2(CustomLine):
    ...
    @staticmethod
    def _find_best_anchor(
        anchors: Sequence[str], fields: Dict[str, ListFieldV2]
    ) -> str:
        """
        Find the anchor with the most rows, in the order specified by `anchors`.
        ...
        anchor_rows = 0
        for field in anchors:
            values: List[ListFieldValueV2] = fields[field].values
            if values and len(values) > anchor_rows:
                anchor_rows = len(values)
                anchor = field
        return anchor
    ...
    def prepare(
        ...
        current_line_number: int = 1
        current_line = CustomLineV2(current_line_number)
        if anchor_field and len(anchor_field.values) > 0:
            current_value = anchor_field.values[0]
            current_line.bbox = extend_bbox(
                current_line.bbox,
    ...
    def get_line_items(
        ...
        :fields: List of field names to reconstruct table with
        """
        line_items: List[CustomLineV2] = []
        fields_to_transform: Dict[str, ListFieldV2] = {}
        for field_name, field_value in fields.items():
            if field_name in field_names:
                fields_to_transform[field_name] = field_value
        anchor = CustomLineV2._find_best_anchor(anchors, fields_to_transform)
        if not anchor:
            print(Warning("Could not find an anchor!"))
            return line_items
        lines_prepared: List[CustomLineV2] = CustomLineV2.prepare(
            anchor, fields_to_transform, height_line_tolerance
        )
