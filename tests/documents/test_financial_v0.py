...
from mindee.documents.financial.financial_v0 import FinancialV0
...
def financial_doc_from_invoice_object():
    ...
    return FinancialV0(api_prediction=json_data["document"]["inference"], page_n=None)
...
def financial_doc_from_receipt_object_all_na():
    ...
    return FinancialV0(api_prediction=json_data["document"]["inference"]["pages"][0])
...
def test__receipt_reconstruct_total_excl_from_total_and_taxes_1(receipt_pred):
    ...
    financial_doc = FinancialV0(receipt_pred)
...
def test__invoice_reconstruct_total_excl_from_total_and_taxes_1(invoice_pred):
    ...
    financial_doc = FinancialV0(invoice_pred)
    assert financial_doc.total_excl.value is None
...
def test_invoice_or_receipt_get_same_field_types(receipt_pred, invoice_pred):
    financial_doc_from_receipt = FinancialV0(receipt_pred)
    financial_doc_from_invoice = FinancialV0(invoice_pred)
