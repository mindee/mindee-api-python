...
from mindee.v1.product.us.bank_check.bank_check_v1 import BankCheckV1
...
def test_default_sample():
    client = Client()
    with open(
        V1_PRODUCT_PATH / "bank_check" / "response_v1" / "default_sample.rst",
        encoding="utf-8",
    ) as rst_file:
        rst_ref = rst_file.read()

    sample = PathInput(
        V1_PRODUCT_PATH / "bank_check" / "default_sample.jpg",
    )
