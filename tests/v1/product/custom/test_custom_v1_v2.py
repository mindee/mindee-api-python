...
def custom_v1_complete_doc() -> Document[CustomV1Document, Page[CustomV1Page]]:
    with open(
        V1_PRODUCT_DATA_DIR / "custom" / "response_v2" / "complete.json"
    ) as json_file:
...
def custom_v1_empty_doc() -> Document[CustomV1Document, Page[CustomV1Page]]:
    with open(
        V1_PRODUCT_DATA_DIR / "custom" / "response_v2" / "empty.json"
    ) as json_file:
        json_data = json.load(json_file)
...
def test_complete_doc(custom_v1_complete_doc) -> None:
    ...
    with open(
        V1_PRODUCT_DATA_DIR / "custom" / "response_v2" / "summary_full.rst"
    ) as doc:
        doc_str = doc.read()
    for field_name, field in document_prediction.fields.items():
        assert len(field_name) > 0
...
def test_complete_page_0(custom_v1_complete_page_0):
    ...
    with open(
        V1_PRODUCT_DATA_DIR / "custom" / "response_v2" / "summary_page0.rst"
    ) as page0:
...
def test_complete_page_1(custom_v1_complete_page_1):
    ...
    with open(
        V1_PRODUCT_DATA_DIR / "custom" / "response_v2" / "summary_page1.rst"
    ) as page1:
