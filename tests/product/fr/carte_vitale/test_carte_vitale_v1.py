...
@pytest.fixture
def complete_doc() -> (
    Document[CarteVitaleV1Document, Page[CarteVitaleV1Document]]
):
    json_data = json.load(
...
def test_complete_page_0(
  complete_page_0: Page[CarteVitaleV1Document]
):
    reference_str = open(
      PRODUCT_DATA_DIR / "carte_vitale" / "response_v1" / "summary_page0.rst",
        "r",
        encoding="utf-8",
    ).read()
