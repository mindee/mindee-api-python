...
def test_default_sample():
    client = Client()
    with open(
        PRODUCT_DATA_DIR / "us_driver_license" / "response_v1" / "default_sample.rst",
        encoding="utf-8",
    ) as rst_file:
        rst_ref = rst_file.read()

    sample = client.source_from_path(
        PRODUCT_DATA_DIR / "us_driver_license" / "default_sample.jpg",
    )
    response = client.parse(DriverLicenseV1, sample)
